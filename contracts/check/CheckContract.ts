...
const ContractTestData = makeTestData<Check<TestedFunctionType>,typeof ContractTestDataDescriptor>(
    ContractTestDataDescriptor,
    ()=>new Check<TestedFunctionType>())
...
const otherReturnvalueRegEx = RegExp(LabelTestdata.runIdentification()+" return value mismatch:.*expected:.2.*actual  :.1", "ms")
export const CheckContract =
    new Contract<typeof Check.prototype.check>()
        ...
        .thenThrow("if there is no ifCalledWith, a 'no ifCalledWith' error is thrown", LabelTestdata.runIdentification() + " no ifcalledWith")
        ...
        .thenThrow("if a return value check fails, a 'return value check did not hold' error is thrown", LabelTestdata.runIdentification() + " fail: return value check did not hold")
        ...
        .thenThrow(
            ...
            LabelTestdata.runIdentification() + "Not the expected exception thrown. Got:Error: first arg cannot be two\nstack:\nError: first arg cannot be two")
        ...
        .thenThrow(
            ...
            LabelTestdata.runIdentification() + " side effect check: logs to console: did not hold")
        ...
        .ifCalledWith(ContractTestData.getContractWithNonDefaultCaseWithARunStored,TestedFunctionTestData.withGlobal)
