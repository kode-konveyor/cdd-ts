...
export class Check<T extends SutType>  {
    ...
    check(contract: ContractEntity<T>, sut: T) {
        console.log("checking",[contract,sut])
        ...
        for(const casename in contract.cases) {
            const thisCase = contract.cases[casename];
    ...
    private handleRun(contract: ContractEntity<T>, currentRun: RunDescriptorEntity<T>) {
        ...
        console.log(currentRun.explanation)
        ...
        try {
            ...
            this.handleException(contract, currentRun, e as Error);
            return 1
        }
    ...
    private handleException(contract: ContractEntity<T>, currentRun: RunDescriptorEntity<T>, catched: Error) {
        ...
        if(!String(catched).match(currentRun.thrown))
            throw new Error(this.caseName(contract)+":Not the expected exception thrown. Got:"+ catched.stack)
    }
